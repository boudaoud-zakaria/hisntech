...
const mockData: Incident[] = [
    { id: 'INC-2490', severity: 'Critical', type: 'Ransomware Attempt', status: 'Investigating', detectedAt: 'Dec 31, 2025', assignedTo: 'S. Miller', actions: 'N/A' },
    { id: 'INC-2491', severity: 'Medium', type: 'Unauthorized Access', status: 'New', detectedAt: 'Dec 31, 2025', assignedTo: 'J. Doe', actions: 'N/A' },
    { id: 'INC-2492', severity: 'High', type: 'Potential Phishing', status: 'Resolved', detectedAt: 'Dec 25, 2025', assignedTo: 'J. Doe', actions: 'N/A' },
    { id: 'INC-2493', severity: 'Low', type: 'Malware Detected', status: 'Resolved', detectedAt: 'Dec 22, 2025', assignedTo: 'J. Doe', actions: 'N/A' },
    { id: 'INC-2494', severity: 'Low', type: 'Malware Detected', status: 'Resolved', detectedAt: 'Dec 22, 2025', assignedTo: 'J. Doe', actions: 'N/A' },
];
...
export default function IncidentsTable() {
    ...
    const filteredData = mockData.filter(incident =>
    ...
    // Grid columns match: 12% 12% 20% 12% 18% 14% 12%
            ...
            <div className="space-y-1 bg-card rounded-2xl px-6 py-4">
                <div className="w-full">
                    <table className="w-full text-left border-collapse table-fixed">
                        <thead>
                            <tr className="text-[10px] font-bold text-gray-500 uppercase tracking-widest px-6">
                                ...
                                {filteredData.length > 0 ? (
                                    filteredData.map((incident, index) => (
                                        <tr key={index} className="hover:bg-white/[0.02] transition-colors group">
                                            <td className="w-[12%] px-6 py-6 text-xs font-medium text-white truncate">{incident.id}</td>
                                            <td className="w-[12%] px-6 py-6 font-medium">
                                                <span className={cn(
                                            ...
                                            <td className="w-[20%] px-6 py-6 text-xs text-gray-300 truncate">{incident.type}</td>
                                            <td className="w-[12%] px-6 py-6 text-xs text-gray-300 truncate">{incident.status}</td>
                                            <td className="w-[18%] px-6 py-6 text-xs text-gray-300 truncate">{incident.detectedAt}</td>
                                            <td className="w-[14%] px-6 py-6 text-xs text-gray-300 font-medium truncate">{incident.assignedTo}</td>
                                            <td className="w-[12%] px-6 py-6 text-xs text-gray-500 uppercase font-bold text-right pr-6 truncate">{incident.actions}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
                                        <td colSpan={7} className="px-6 py-20 text-center text-xs text-gray-500 italic">
                ...
                <div className="px-6 py-8 flex flex-wrap items-center justify-between gap-4">
                    <span className="text-xs text-gray-500">
                        Showing <span className="text-white font-medium">1</span> to <span className="text-white font-medium">{filteredData.length}</span> of <span className="text-white font-medium">1,250</span> incidents
                    </span>
